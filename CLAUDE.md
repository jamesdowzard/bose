...
- `cli/Presence.swift` -- `bose presence [--timeout s] [--json]`: PASSIVE BLE advert scan — receive-only, zero packets to the headphones, so it's safe from a non-slot Mac and exempt from the don't-probe rule. verBosita advertises ~4/sec while on (name + Bose company ID 0x009E; pure matcher `isBoseAdvert` in Parsers.swift, unit-tested; mfr payload logged 2026-07-18 `9e00002c06dfff8ab42110` — battery MAY be in it, correlate before trusting). The app piggybacks one bounded scan on each unreachable refresh (never a poll) to add "headphones on & nearby / not seen nearby" to the staleness banner (`nearbyPresence`).
