...
back) AND the headphones have dropped off the Mac AND the Mac was your last active
source, `hammerspoon/bose.lua`'s `walkbackTap` runs `bose connect mac`. It does NOT
reintroduce #61/#62 because it never probes the headphones over RFCOMM to decide — the
"in range?" check is `blueutil --is-connected` (channel-free) and the "want the Mac?"
gate is the `~/.config/bose/last-active-mac` flag the CLI writes on connect/swap and
clears on any non-Mac connect or explicit Mac disconnect. Event-driven (`hs.eventtap`),
no timer, no headphone poll, multipoint stays on. Pure decision lives in
`hammerspoon/reconnect_decision.lua` (`lua reconnect_decision_test.lua`).
...
- **NEVER auto-reconnect A2DP by probing** — an RFCOMM/ACL probe to decide fights user device switches (#61-#64). The ONE allowed auto-reconnect is Hammerspoon's walk-back watcher, which decides via channel-free `blueutil --is-connected` + the last-active-mac flag and only ever opens RFCOMM once it has decided to connect (see connectDevice Behaviour → walk-back exception)
