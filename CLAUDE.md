...
**Exception — walk-back auto-reconnect (Mac, Hammerspoon).** The ONE sanctioned
auto-reconnect: when input resumes after a >= 150s idle gap (you left the desk and came
back) AND the headphones have dropped off the Mac AND the Mac was your last active source
AND that last-active signal is still **fresh**, `hammerspoon/bose.lua`'s `walkbackTap`
runs `bose connect mac`. It does NOT reintroduce #61/#62 because it never probes the
headphones over RFCOMM to decide — the "in range?" check is `blueutil --is-connected`
(channel-free) and the "want the Mac?" gate is the `~/.config/bose/last-active-mac` flag
the CLI writes on connect/swap (`.active` only) and clears on any non-Mac connect or
explicit Mac disconnect. **Staleness TTL (#139 finding #1):** because the flag is written
ONLY by the Mac CLI, a source switch made on the **phone** side never clears it, so it can
go stale-set and a walk-back would then STEAL audio off the phone. The gate therefore also
requires `flagAge < WALKBACK_FLAG_TTL` (the flag's mtime = when it was set; default 8h ≈ a
workday). This **narrows** that hole (a flag left stale overnight / across sessions expires)
but does NOT fully close it — a same-session phone-switch leaves a still-fresh flag; closing
it entirely would need a forbidden RFCOMM read. Event-driven (`hs.eventtap`), no timer, no
headphone poll, multipoint stays on. `maybeReconnect` checks blueutil's exit code + launch
and logs a read/launch failure rather than dying silently (#139 finding #2). Pure decision
lives in `hammerspoon/reconnect_decision.lua` (`lua reconnect_decision_test.lua`).
...
- **NEVER auto-reconnect A2DP by probing** — an RFCOMM/ACL probe to decide fights user device switches (#61-#64). The ONE allowed auto-reconnect is Hammerspoon's walk-back watcher, which decides via channel-free `blueutil --is-connected` + the last-active-mac flag (gated on a freshness TTL so a phone-side switch's stale flag can't steal audio back — #139) and only ever opens RFCOMM once it has decided to connect (see connectDevice Behaviour → walk-back exception)
